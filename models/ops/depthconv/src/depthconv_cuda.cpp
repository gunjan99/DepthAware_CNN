...
int depthconv_forward_cuda(THCudaTensor *input, THCudaTensor *input_depth, THCudaTensor *weight, THCudaTensor *bias, THCudaTensor *output,
  ...
  if (input->dim() == 3) {                                                        // input->nDimension => input->dim()
    // Force batch
    batch = 0;
    THCudaTensor_resize4d(state, input, 1, input->size(0), input->size(1),          // input->size[0/1] => input->size(0/1)
                          input->size(2));                                          // input->size[2] => input->size(2)
    THCudaTensor_resize4d(state, input_depth, 1, input_depth->size(0), input_depth->size(1),    // input_depth->size[0/1] => input_depth->size(0/1)
                          input_depth->size(2));                                    // input_depth->size[2] => input_depth->size(2)
  }
...
int depthconv_backward_input_cuda(
  ...
  if (input->dim() == 3) {                                                  // input->nDimension => input->dim()
    // Force batch
    batch = 0;
    THCudaTensor_resize4d(state, input, 1, input->size(0), input->size(1),  // input->size[0] => input->size(0); input->size[1] => input->size(1)
                          input->size(2));                                  // input->size[2] => input->size(2)
    THCudaTensor_resize4d(state, gradOutput, 1, gradOutput->size(0),        // gradOutput->size[0] => gradOutput->size(0)
                          gradOutput->size(1), gradOutput->size(2));        // gradOutput->size[1/2] => gradOutput->size(1/2)
  }
